...
public class WheelView extends View {
    ...
    public WheelView(Context context, AttributeSet attrs) {
        super(context, attrs);
        textColorOut = getResources().getColor(R.color.text_tips);
        textColorCenter = getResources().getColor(R.color.text_content);
        dividerColor = getResources().getColor(R.color.activity_line);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (label != null) {
            ...
            //靠右并留出空隙
            paintCenterText.setTextSize(DensityUtil.dp2px(context, 11));
            canvas.drawText(label, drawRightContentStart - CENTERCONTENTOFFSET - 6, centerY - 3, paintCenterText);
            paintCenterText.setTextSize(textSize);
        }
        counter = 0;
        while (counter < itemsVisible) {
